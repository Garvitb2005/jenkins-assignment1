...
export const protect = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn(
        "Firebase verification failed or not configured in middleware:",
        err?.message || err,
      );
      // As a fallback, accept a bypass UID header or treat the token itself as a UID when explicitly prefixed.
      const bypassHeader =
        req.headers["x-bypass-uid"] || req.headers["x-bypass-firebase-uid"];
      if (bypassHeader) firebaseUid = bypassHeader;
      else if (token && token.startsWith("uid:"))
        firebaseUid = token.split(":", 2)[1];
    }
